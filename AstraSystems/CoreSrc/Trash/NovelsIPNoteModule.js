class NovelsIPNoteModule {
    constructor() {
        console.log("NovelsIPNoteModule is instantiated.");
    }
     ...
     * @param {int} hierarchy 
     * @returns {NovelsIPNote} instance
     */
    createInstance(MyNote, dv, file, childLimiter = 1, relatedLimiter = 1, hierarchy = 1) {
        const NovelsIPNote = this.getNoteClass(MyNote);
        return new NovelsIPNote(dv, file, childLimiter, relatedLimiter, hierarchy);
    }

    /**
     * @extends MyNote from ./MyNoteModule.js
     * @param {MyNote} MyNote Class
     * @returns {NovelsIPNote} class
     */
    getNoteClass(MyNote) {
        return class NovelsIPNote extends MyNote {
